...
output "cache_bucket_name" {
  ...
  value       = var.codebuild_enabled && local.s3_cache_enabled ? join("", aws_s3_bucket.cache_bucket.*.bucket) : "UNSET"
}

output "cache_bucket_arn" {
  description = "Cache S3 bucket ARN"
  value       = var.codebuild_enabled && local.s3_cache_enabled ? join("", aws_s3_bucket.cache_bucket.*.arn) : "UNSET"
}
